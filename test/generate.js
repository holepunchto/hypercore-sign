...
test('generate - password too short', async (t) => {
  ...
  g.on('close', (code) => {
    t.is(code, 1, 'Key creation successfully errored')
  })
...
test('generate - key with dots', async (t) => {
  ...
  g.on('close', (code) => {
    t.is(code, 0, 'Key successfully created')
  })
...
test('generate - help', async (t) => {
  ...
  g.on('close', (code) => {
    t.is(code, 0, 'process exit code')
    t.ok(message.includes('hypercore-sign'))
    t.ok(message.includes('generate'))
    t.ok(message.includes('Flags'))
  })

  await t.execution(dummyUser(g), /exit code/)
