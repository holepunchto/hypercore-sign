...
const { encrypt, decrypt } = require('pwd-encrypt')
...
async function generateKeys (pwd) {
  ...
  let result = null
  try {
    result = await encrypt(secretKey, pwd, { kdfParams })
  } finally {
  ...
  return {
    publicKey,
    secretKey: result.encryptedData,
    id: result.id
  }
}
...
async function sign (signables, keyBuffer, pwd) {
  sodium.sodium_mprotect_readwrite(pwd)

  let result = null
  try {
    result = await decrypt(keyBuffer, pwd)
  } finally {
    sodium.sodium_memzero(pwd)
    sodium.sodium_free(pwd)
  }
